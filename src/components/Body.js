    ...
    return lisOfRestaurants.length === 0 ? <Shimmer/> : (
        <div>
            <div className="flex">
                ...
                <button onClick = {()=>{
                    const filteredRestaurants = lisOfRestaurants.filter((restaurant)=>restaurant.info.name.toLowerCase().includes(searchText.toLowerCase()))
                    setFilteredRestaurants(filteredRestaurants)
                }} className="text-md p-1 m-4 h-8 w-24 bg-indigo-600 rounded-md text-white">Search</button>
                </div>
                
                <button onClick ={()=>{
                    const filteredRestaurants = lisOfRestaurants.filter((restaurant)=>(restaurant.info.avgRating >4 ))
                    setFilteredRestaurants(filteredRestaurants)
